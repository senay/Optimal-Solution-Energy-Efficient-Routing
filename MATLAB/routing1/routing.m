...
maxPower = 230;                  %maximum power that a node shold consume
...
while(currentTime < simulationTime),
    ...
    if (currentTime > 0)
        powerConsumption = evalin('base', 'powerConsumption');%read current power cunsumption of nodes from workspace
    end
    ...
    for i = 1:N
        if (any(availableActive(i,:))&&(powerConsumption(i) > maxPower))% if the node has at least one link
            for j = 1:N
                available(i,j) = 0;
                available(j,i) = 0;
    ...
    flowArrival(available, availableActive, flowRate, N, currentTime, mu);
    eventTimeLast = eventTimeNow;
