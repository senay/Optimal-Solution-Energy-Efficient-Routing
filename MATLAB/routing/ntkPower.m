function ntkPwr = ntkPower(uData, N)
...
pIdle = 200;           % idle power of a node in watts
...
Esf = eRx + eXfer + eTx; %packet store and forward energy
    ...
    for i = 1:N
        if(any(load(i,:)))% if the node has at least one link
            ...
            for j = 1:N
                if(load(i,j)>0)
                    ntkPwr = ntkPwr + load(i,j)*(10.^3)*(Ep + Esf)/(10.^9);
                    powerConsumption(i) = nodePwr + load(i,j)*(10.^3)*(Ep + Esf)/(10.^9);
                end
            end
        end
    end
    assignin('base','powerConsumption',powerConsumption);
